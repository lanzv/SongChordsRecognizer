...
def stft_chromagram(waveform, sample_rate:int=44100, nfft:int=2**14, hop_length:int=4410):
    """
    ...
    stft_chroma : float array
        stft chromagram
    """
    stft_chroma = librosa.feature.chroma_stft(y=waveform, sr=sample_rate, n_chroma=12, hop_length=hop_length, n_fft=nfft)
...
def cqt_chromagram(waveform, sample_rate:int=44100, nfft:int=2**14, hop_length:int=4410):
    """
    Generate chromagram from audio.
    ...
    cqt_chroma : float array
        cqt chromagram
    """
    cqt_chroma = librosa.feature.chroma_cqt(y=waveform, sr=sample_rate, hop_length=hop_length)
