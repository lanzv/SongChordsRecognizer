...
def save_preprocessed_Isophonics(args):
    ...
    data = IsophonicsDataset(args.audio_directory, args.annotations_directory, sample_rate=args.sample_rate)
    data.save_preprocessed_dataset(dest=args.billboard_prep_dest, window_size=args.window_size, flattened_window=args.flattened_window, ms_intervals=args.ms_intervals, to_skip=args.top_skip, norm_to_C=args.norm_to_C, spectrogram_generator=spectrogram)
...
parser.add_argument("--dataset", default="isophonics", type=int, help="Dataset we want to preprocess, {isophonics, billboard}")
#           Isophonics
...
#           Billboard
parser.add_argument("--n_frames", default=500, type=int, help="Length of song subsequence we are consinder when predicting chords to keep some context.")
